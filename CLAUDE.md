...
- **Quellinhalt:** `content/source/parasitologie-quelle.md` (aus `Parasiten.md.md` kopieren). ~49 Helminthen-Monografien (Trematoden/Zestoden/Nematoden), auf Deutsch.
...
2. **Keine Halluzination im Inhalt.** Jede Frage muss sich aus `content/source/...` belegen lassen. Pflichtfelder pro Frage: `explanation` (Begründung, Deutsch) und `source` (Kapitel/Abschnitt aus der Quelle). Unsichere Fragen: `"needsReview": true`.
...
2. **Fragenbank generieren** aus der Quelle (400–800 Fragen, valide, `needsReview` markiert).
